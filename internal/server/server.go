...
func New(cfg config.Config, d *db.DB) (*gin.Engine, *stats.Service) {
	...
	r.GET("/llm-gateway-lite/:path/responses", prox.Info)
	r.GET("/llm-gateway-lite/:path/v1/responses", prox.Info)
	...
	r.POST("/llm-gateway-lite/:path/responses/responses", prox.Responses)
	r.POST("/llm-gateway-lite/:path/v1/responses/responses", prox.Responses)
