									...
									<div className="p-5 space-y-4">
											...
											<div className="flex items-start gap-3 rounded-lg border border-[var(--color-hairline)] bg-[var(--color-surface-soft)] px-4 py-3">
												...
												<div>
													<p className="text-xs font-semibold text-[var(--color-ink)]">
														{t.linkEditor.autoDetectTitle}
													</p>
													<p className="mt-0.5 text-xs text-[var(--color-muted)]">
														{t.linkEditor.autoDetectDesc}
													</p>
												</div>
											</div>
										)}
										...
										<div className="grid grid-cols-2 gap-4">
											<div className="space-y-1.5">
												<label className={labelCls}>
													{t.linkEditor.fallbackModelLabel}
												</label>
												<input
													value={entry.fallback_model}
													onChange={(e) =>
														updateChain(i, "fallback_model", e.target.value)
													}
													placeholder={t.linkEditor.fallbackModelPlaceholder}
													className={fieldCls}
												/>
											</div>
											<div className="space-y-1.5">
												<label className={labelCls}>
													{hasGlobalKey
														? t.linkEditor.apiKeyOverrideLabel
														: t.linkEditor.apiKeyLabel}
												</label>
												<input
													type="text"
													autoComplete="off"
													...
													value={entry.api_key || ""}
													onChange={(e) =>
														updateChain(i, "api_key", e.target.value)
													}
													placeholder={
														hasGlobalKey
															? t.linkEditor.apiKeyPlaceholderHasGlobal
															: t.linkEditor.apiKeyPlaceholder
													}
													className={fieldCls}
												/>
											</div>
